<?php
	...
	if(empty($_REQUEST['cl']) || empty($_REQUEST['f']))
	{
		...
		$pdf = new FPDF('P', 'mm', array(48, 100));
		$pdf->AddPage();
		$pdf->SetMargins(0, 0, 0);
		$pdf->SetTitle("Ventas");
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->Cell(29, 5, utf8_decode($resultado['nombre']), 0, 1, 'C');
		$pdf->Ln();
		...
		$pdf->SetFont('Arial', '', 7);
		$pdf->Cell(20, 5, $result_venta['fecha'], 0, 1, 'C');

		$pdf->SetFont('Arial', 'B', 7);
		$pdf->Cell(10, 5, "Ticked: ", 0, 0, 'L');
		$pdf->SetFont('Arial', '', 7);
		
		$pdf->Cell(3, 5, $noFactura, 0, 1, 'L');
		$pdf->SetFont('Arial', 'B', 7);
		...
		// $pdf->SetFont('Arial', '', 7);
		if ($_GET['cl'] == 1) {
		$pdf->Cell(30, 5, utf8_decode("Público en general"), 0, 0, 'L');
		...
		$pdf->SetFont('Arial', 'B', 7);
		$pdf->Cell(22, 5, 'Nombre', 0, 0, 'L');
		$pdf->Cell(8, 5, 'Cant', 0, 0, 'L');
		$pdf->Cell(10, 5, 'Precio', 0, 0, 'L');
		$pdf->Cell(10, 5, 'Total', 0, 1, 'L');
		$pdf->SetFont('Arial', '', 7);
		while ($row = mysqli_fetch_assoc($productos)) {
			$pdf->Cell(23, 5, utf8_decode($row['descripcion']), 0, 0, 'L');
			$pdf->Cell(5, 5, $row['cantidad'], 1, 0, 'R');
			$pdf->Cell(10, 5, number_format($row['precio'], 0, '.', ','), 1, 0, 'L');
			$importe = number_format($row['cantidad'] * $row['precio'], 0, '.', ',');
			$pdf->Cell(10, 5, $importe, 0, 1, 'L');
		}
		$pdf->Ln();
		$pdf->SetFont('Arial', 'B', 10);

		$pdf->Cell(35, 5, 'Total: ' . number_format($result_venta['totalfactura'], 2, '.', ','), 0, 1, 'R');
		$pdf->Ln();
		$pdf->SetFont('Arial', '', 7);
		$pdf->Cell(48, 5, utf8_decode("Gracias por su preferencia"), 0, 1, 'C');
